...
router.post('/run', async (req, res) => {
  ...
  try {
    ensureGapTable();
    const sys = 'You are an assistant for the "No audit logging" feature in project AITelecomCustomerExperienceManager. Respond as strict JSON.';
    ...
    if (pool) {
      try { await pool.query('INSERT INTO gap_features(feature_slug, project, input, output) VALUES ($1,$2,$3,$4)', ['gap-no-audit-logging', 'AITelecomCustomerExperienceManager', JSON.stringify(input), JSON.stringify(out)]); } catch (_) {}
    }
