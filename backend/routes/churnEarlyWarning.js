...
// Churn early-warning system with real-time scoring and retention campaigns
// Feature: churn-early-warning
...
router.post('/analyze', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Churn early-warning system with real-time scoring and retention campaigns.\nUser input: ${JSON.stringify(payload).slice(0, 3500)}\n\nProduce JSON in this shape:\n{\n  "summary": "...",\n  "findings": [...],\n  "recommendations": [...],\n  "score": 0.0,\n  "details": {}\n}`
    );
    const uid = req.user && (req.user.id || req.user.userId || req.user.user_id);
    await persist(uid, 'churn-early-warning', payload, result);
    res.json({ success: true, feature: 'churn-early-warning', ...result });
  } catch (err) {
    console.error('churn-early-warning analyze error:', err.message);
...
router.post('/batch', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Churn early-warning system with real-time scoring and retention campaigns. Batch of ${items.length} items.\nItems: ${JSON.stringify(items).slice(0, 3500)}\nReturn JSON: { results:[{index, score, summary, recommendation}], aggregate:{count, mean_score, top_concerns:[]} }`,
      { maxTokens: 3072 }
    );
    res.json({ success: true, feature: 'churn-early-warning', count: items.length, ...result });
  } catch (err) {
...
router.get('/info', auth, (req, res) => {
  res.json({ feature: 'churn-early-warning', title: 'Churn early-warning system with real-time scoring and retention campaigns', project: 'AITelecomCustomerExperienceManager' });
});
