...
// Call quality root-cause analysis correlating with network/customer factors
// Feature: call-quality-rca
...
router.post('/analyze', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Call quality root-cause analysis correlating with network/customer factors.\nUser input: ${JSON.stringify(payload).slice(0, 3500)}\n\nProduce JSON in this shape:\n{\n  "summary": "...",\n  "findings": [...],\n  "recommendations": [...],\n  "score": 0.0,\n  "details": {}\n}`
    );
    const uid = req.user && (req.user.id || req.user.userId || req.user.user_id);
    await persist(uid, 'call-quality-rca', payload, result);
    res.json({ success: true, feature: 'call-quality-rca', ...result });
  } catch (err) {
    console.error('call-quality-rca analyze error:', err.message);
...
router.post('/batch', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Call quality root-cause analysis correlating with network/customer factors. Batch of ${items.length} items.\nItems: ${JSON.stringify(items).slice(0, 3500)}\nReturn JSON: { results:[{index, score, summary, recommendation}], aggregate:{count, mean_score, top_concerns:[]} }`,
      { maxTokens: 3072 }
    );
    res.json({ success: true, feature: 'call-quality-rca', count: items.length, ...result });
  } catch (err) {
...
router.get('/info', auth, (req, res) => {
  res.json({ feature: 'call-quality-rca', title: 'Call quality root-cause analysis correlating with network/customer factors', project: 'AITelecomCustomerExperienceManager' });
});
