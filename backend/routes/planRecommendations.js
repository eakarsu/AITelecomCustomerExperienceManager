...
router.get('/', auth, async (req, res) => {
  try {
    ...
    const countResult = await pool.query('SELECT COUNT(*) FROM plan_recommendations');
    const total = parseInt(countResult.rows[0].count);
    const result = await pool.query('SELECT * FROM plan_recommendations ORDER BY created_at DESC LIMIT $1 OFFSET $2', [limit, offset]);
...
router.post('/:id/recommend', auth, aiRateLimiter, async (req, res) => {
  try {
    const pool = req.app.get('db');
    const result = await pool.query('SELECT * FROM plan_recommendations WHERE id = $1', [req.params.id]);
    if (result.rows.length === 0) return res.status(404).json({ error: 'Record not found' });
