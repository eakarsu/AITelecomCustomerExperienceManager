...
router.get('/', auth, async (req, res) => {
  try {
    ...
    const countResult = await pool.query('SELECT COUNT(*) FROM proactive_issues');
    const total = parseInt(countResult.rows[0].count);
    const result = await pool.query('SELECT * FROM proactive_issues ORDER BY created_at DESC LIMIT $1 OFFSET $2', [limit, offset]);
