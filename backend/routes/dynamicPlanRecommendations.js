...
// Dynamic plan recommendations from usage patterns + account risk
// Feature: dynamic-plan-recommendations
...
router.post('/analyze', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      'You are an expert assistant for the "AITelecomCustomerExperienceManager" platform. Always return strict JSON only — no markdown, no commentary.',
      `Feature: Dynamic plan recommendations from usage patterns + account risk.\nUser input: ${JSON.stringify(payload).slice(0, 3500)}\n\nProduce JSON in this shape:\n{\n  "summary": "...",\n  "findings": [...],\n  "recommendations": [...],\n  "score": 0.0,\n  "details": {}\n}`
    );
    const uid = req.user && (req.user.id || req.user.userId || req.user.user_id);
    await persist(uid, 'dynamic-plan-recommendations', payload, result);
    res.json({ success: true, feature: 'dynamic-plan-recommendations', ...result });
  } catch (err) {
    console.error('dynamic-plan-recommendations analyze error:', err.message);
...
router.post('/batch', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      'You analyze batches of items for "AITelecomCustomerExperienceManager". Return strict JSON only.',
      `Feature: Dynamic plan recommendations from usage patterns + account risk. Batch of ${items.length} items.\nItems: ${JSON.stringify(items).slice(0, 3500)}\nReturn JSON: { results:[{index, score, summary, recommendation}], aggregate:{count, mean_score, top_concerns:[]} }`,
      { maxTokens: 3072 }
    );
    res.json({ success: true, feature: 'dynamic-plan-recommendations', count: items.length, ...result });
  } catch (err) {
...
router.get('/history', auth, async (req, res) => {
  try {
    ...
    const { rows } = await pool.query(
      ...
      [uid, 'dynamic-plan-recommendations']
    );
...
router.get('/info', auth, (req, res) => {
  res.json({ feature: 'dynamic-plan-recommendations', title: 'Dynamic plan recommendations from usage patterns + account risk', project: 'AITelecomCustomerExperienceManager' });
});
