...
// Composite customer health score (payment + satisfaction + churn + NPS)
...
router.post('/analyze', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Composite customer health score (payment + satisfaction + churn + NPS).\nUser input: ${JSON.stringify(payload).slice(0, 3500)}\n\nProduce JSON in this shape:\n{\n  "summary": "...",\n  "findings": [...],\n  "recommendations": [...],\n  "score": 0.0,\n  "details": {}\n}`
    );
    const uid = req.user && (req.user.id || req.user.userId || req.user.user_id);
    await persist(uid, 'customer-health-score', payload, result);
    res.json({ success: true, feature: 'customer-health-score', ...result });
  } catch (err) {
    console.error('customer-health-score analyze error:', err.message);
...
router.post('/batch', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Composite customer health score (payment + satisfaction + churn + NPS). Batch of ${items.length} items.\nItems: ${JSON.stringify(items).slice(0, 3500)}\nReturn JSON: { results:[{index, score, summary, recommendation}], aggregate:{count, mean_score, top_concerns:[]} }`,
      { maxTokens: 3072 }
    );
    res.json({ success: true, feature: 'customer-health-score', count: items.length, ...result });
  } catch (err) {
...
router.get('/info', auth, (req, res) => {
  res.json({ feature: 'customer-health-score', title: 'Composite customer health score (payment + satisfaction + churn + NPS)', project: 'AITelecomCustomerExperienceManager' });
});
