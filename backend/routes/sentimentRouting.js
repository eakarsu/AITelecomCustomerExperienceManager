...
// Sentiment-driven routing escalating negative calls to specialists
...
router.post('/analyze', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Sentiment-driven routing escalating negative calls to specialists.\nUser input: ${JSON.stringify(payload).slice(0, 3500)}\n\nProduce JSON in this shape:\n{\n  "summary": "...",\n  "findings": [...],\n  "recommendations": [...],\n  "score": 0.0,\n  "details": {}\n}`
    );
    const uid = req.user && (req.user.id || req.user.userId || req.user.user_id);
    await persist(uid, 'sentiment-routing', payload, result);
    res.json({ success: true, feature: 'sentiment-routing', ...result });
  } catch (err) {
    console.error('sentiment-routing analyze error:', err.message);
...
router.post('/batch', auth, async (req, res) => {
  ...
  try {
    const result = await callOpenRouter(
      ...
      `Feature: Sentiment-driven routing escalating negative calls to specialists. Batch of ${items.length} items.\nItems: ${JSON.stringify(items).slice(0, 3500)}\nReturn JSON: { results:[{index, score, summary, recommendation}], aggregate:{count, mean_score, top_concerns:[]} }`,
      { maxTokens: 3072 }
    );
    res.json({ success: true, feature: 'sentiment-routing', count: items.length, ...result });
  } catch (err) {
...
router.get('/info', auth, (req, res) => {
  res.json({ feature: 'sentiment-routing', title: 'Sentiment-driven routing escalating negative calls to specialists', project: 'AITelecomCustomerExperienceManager' });
});
