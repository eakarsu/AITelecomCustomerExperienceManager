...
// Feature: No webhooks for outage events
...
router.post('/run', async (req, res) => {
  ...
  try {
    ensureGapTable();
    const sys = 'You are an assistant for the "No webhooks for outage events" feature in project AITelecomCustomerExperienceManager. Respond as strict JSON.';
    const user = `Feature request: No webhooks for outage events\n\nUser input JSON:\n` + JSON.stringify(input).slice(0, 4000) + '\n\nReturn JSON with summary, findings array, recommendations array.';
    const out = await callOpenRouter(sys, user);
    if (pool) {
      try { await pool.query('INSERT INTO gap_features(feature_slug, project, input, output) VALUES ($1,$2,$3,$4)', ['gap-no-webhooks-for-outage-events', 'AITelecomCustomerExperienceManager', JSON.stringify(input), JSON.stringify(out)]); } catch (_) {}
    }
    res.json({ success: true, feature: 'gap-no-webhooks-for-outage-events', kind: 'gap_non_ai', result: out });
  } catch (err) {
...
router.get('/history', async (req, res) => {
  ...
  try {
    ensureGapTable();
    const { rows } = await pool.query('SELECT id, input, output, created_at FROM gap_features WHERE feature_slug = $1 ORDER BY created_at DESC LIMIT 50', ['gap-no-webhooks-for-outage-events']);
