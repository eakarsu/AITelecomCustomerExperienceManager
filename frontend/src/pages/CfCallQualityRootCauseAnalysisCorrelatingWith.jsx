...
export default function CfCallQualityRootCauseAnalysisCorrelatingWith() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-call-quality-root-cause-analysis-correlating-with-network-customer/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-call-quality-root-cause-analysis-correlating-with-network-customer', project: 'AITelecomCustomerExperienceManager' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-call-quality-root-cause-analysis-correlating-with-network-customer/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Call quality root-cause analysis correlating with network/customer factors</h1>
        <p className="text-slate-500 text-sm mt-1">Batch 08 · cfs · AITelecomCustomerExperienceManager</p>
      </div>
