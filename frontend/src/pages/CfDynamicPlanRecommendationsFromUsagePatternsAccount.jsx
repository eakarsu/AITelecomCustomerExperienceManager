...
export default function CfDynamicPlanRecommendationsFromUsagePatternsAccount() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-dynamic-plan-recommendations-from-usage-patterns-account-risk/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-dynamic-plan-recommendations-from-usage-patterns-account-risk', project: 'AITelecomCustomerExperienceManager' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-dynamic-plan-recommendations-from-usage-patterns-account-risk/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Dynamic plan recommendations from usage patterns + account risk</h1>
