...
export default function CfCompositeCustomerHealthScorePaymentSatisfactionChurn() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-composite-customer-health-score-payment-satisfaction-churn-nps/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-composite-customer-health-score-payment-satisfaction-churn-nps', project: 'AITelecomCustomerExperienceManager' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-composite-customer-health-score-payment-satisfaction-churn-nps/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Composite customer health score (payment + satisfaction + churn + NPS)</h1>
