...
export default function GapCriticalNoAiEndpointsForChurnPrediction() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/gap-critical-no-ai-endpoints-for-churn-prediction-sentiment/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'gap-critical-no-ai-endpoints-for-churn-prediction-sentiment', project: 'AITelecomCustomerExperienceManager' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/gap-critical-no-ai-endpoints-for-churn-prediction-sentiment/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Critical: No AI endpoints for churn prediction, sentiment analysis, call quality assessment, NPS prediction (data routes exist but no ML)</h1>
        <p className="text-slate-500 text-sm mt-1">Batch 08 · gap_ai · AITelecomCustomerExperienceManager</p>
      </div>
