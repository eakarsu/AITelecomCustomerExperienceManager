...
export default function CfSentimentDrivenRoutingEscalatingNegativeCallsTo() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-sentiment-driven-routing-escalating-negative-calls-to-specialists/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-sentiment-driven-routing-escalating-negative-calls-to-specialists', project: 'AITelecomCustomerExperienceManager' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-sentiment-driven-routing-escalating-negative-calls-to-specialists/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Sentiment-driven routing escalating negative calls to specialists</h1>
