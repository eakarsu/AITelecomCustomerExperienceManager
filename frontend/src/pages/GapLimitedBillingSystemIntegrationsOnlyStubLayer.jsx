...
export default function GapLimitedBillingSystemIntegrationsOnlyStubLayer() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/gap-limited-billing-system-integrations-only-stub-layer-no/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'gap-limited-billing-system-integrations-only-stub-layer-no', project: 'AITelecomCustomerExperienceManager' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/gap-limited-billing-system-integrations-only-stub-layer-no/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Limited billing system integrations (only stub layer; no Comtech, Openet)</h1>
