...
export default function CfChurnEarlyWarningSystemWithRealTime() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/cf-churn-early-warning-system-with-real-time-scoring-and-retention/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'cf-churn-early-warning-system-with-real-time-scoring-and-retention', project: 'AITelecomCustomerExperienceManager' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/cf-churn-early-warning-system-with-real-time-scoring-and-retention/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Churn early-warning system with real-time scoring and retention campaigns</h1>
