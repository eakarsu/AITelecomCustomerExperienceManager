...
export default function GapNoCorrelationEngineBetweenNetworkPerformanceAnd() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/gap-no-correlation-engine-between-network-performance-and-satisfaction/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'gap-no-correlation-engine-between-network-performance-and-satisfaction', project: 'AITelecomCustomerExperienceManager' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/gap-no-correlation-engine-between-network-performance-and-satisfaction/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">No correlation engine between network performance and satisfaction</h1>
        <p className="text-slate-500 text-sm mt-1">Batch 08 · gap_non_ai · AITelecomCustomerExperienceManager</p>
      </div>
