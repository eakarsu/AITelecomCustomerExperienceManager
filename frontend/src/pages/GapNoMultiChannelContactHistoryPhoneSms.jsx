...
export default function GapNoMultiChannelContactHistoryPhoneSms() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/gap-no-multi-channel-contact-history-phone-sms-email-chat/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'gap-no-multi-channel-contact-history-phone-sms-email-chat', project: 'AITelecomCustomerExperienceManager' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/gap-no-multi-channel-contact-history-phone-sms-email-chat/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">No multi-channel contact history (phone, SMS, email, chat unified)</h1>
