...
export default function GapLimitedNotificationsOneReferenceOnlyNotA() {
  ...
  const submit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/api/gap-limited-notifications-one-reference-only-not-a-full/run`, {
        method: 'POST', headers: getHeaders(),
        body: JSON.stringify({ prompt: input, feature: 'gap-limited-notifications-one-reference-only-not-a-full', project: 'AITelecomCustomerExperienceManager' })
      })
  ...
  const loadHistory = async () => {
    try {
      const res = await fetch(`${API_BASE}/api/gap-limited-notifications-one-reference-only-not-a-full/history`, { headers: getHeaders() })
      const data = await res.json()
  ...
  return (
    <div className="max-w-4xl mx-auto p-4">
      <div className="mb-4">
        <h1 className="text-2xl font-bold text-slate-800">Limited notifications (one reference only — not a full subsystem)</h1>
