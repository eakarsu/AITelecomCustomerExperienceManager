...
export default function CustomerHealthScore() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const res = await axios.post(`${API_URL}/ai/customer-health-score`, {
      ...
      if (status === 503) {
        ...
        setError(err.response?.data?.error || err.message || 'Health score failed');
      }
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div>
      ...
      <p style={{ marginBottom: 20, color: '#999' }}>
        Composite score across loyalty, financial, engagement, satisfaction, and service dimensions.
      </p>
      ...
      {result && (
        <div className="card" style={{ padding: 24 }}>
          <h3 style={{ marginBottom: 14 }}>Health Score</h3>
